...
async def on_message(message):
    ...
    if str(author) != BOT_NAME:
        ...
        # about
            ...
            await message.channel.send('I was made by Ansh Gandhi and Jonathan Ma from FTC Team 7303 RoboAvatars.\nMy source code is [here](https://github.com/norcalbotftc/NorCalBot).\nPlease support my creators by downloading the RoboAvatars Ultimate Goal Scoring app [here](https://tinyurl.com/UltimateGoalRA).\nMy data comes from [NorCal FTC](https://www.norcalftc.org), [FIRST API](https://ftc-events.firstinspires.org/services/API), and [FTC Stats API](https://dynamic.jackcrane.rocks/api/ftcstats/docs.php).')
        ...
        # advancement
            ...
            if lookup[0] != '$advancement' or len(lookup) != 2:
                ...
                if id not in advancement or len(advancement[id]) == 0:
                    ...
                    count = 0
                    for slot in range(len(advancement[id])):
                        team = advancement[id][slot]
                        if team != 0:
                            info.add_field(name = str(count + 1) + ') ' + slots[slot] + ('*' if team in qualified[id] else ''), value = str(team) + ' (' + teams[team][3] + ')')
                            count += 1
