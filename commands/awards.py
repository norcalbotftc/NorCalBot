    ...
    if lookup[0] != '$awards' or len(lookup) != 2:
        ...
        try:
            ...
            if len(apiInfo) == 0:
                ...
                for data in apiInfo:
                    info.add_field(name = data['name'], value = str(data['teamNumber']) + ' (' + db.teams[data['teamNumber']][3] + ')')
...
awards = Command('awards', '$awards [competition id]', 'Shows the rankings at a competition', check, command)
