...
def check(content, author):
    return '$regions' in content

async def command(content, author, message, commands, client):
    output = ''
    for region in REGIONS.keys():
        output += region + ': ' + REGIONS[region].title() + ', '
    await message.channel.send('FTC Stats API Regions:\n' + output[0:-2])

regions = Command('regions', '$regions', 'Shows supported regions for $topteams and $highscores', check, command)
